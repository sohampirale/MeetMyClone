...
class Browser(ABC):
    ...
    async def _async_screenshot(self, action: ScreenshotAction) -> Dict[str, Any]:
     ...
     try:
         ...
         # Decide path (same logic as before)
         if not action.path:
         ...
         # Capture bytes in memory AND save to disk
         bytes_data = await page.screenshot(path=path)  # Playwright returns bytes

         # Encode to base64 for in-memory use
         b64 = base64.b64encode(bytes_data).decode("utf-8")

         return {
            "status": "success",
            "content": [
                {"text": f"Screenshot saved as {path}"},
                {"type": "image", "data": b64},
            ],
            "file_path": path,
            "image_base64": b64,
